class AddMoreIndexesToFavourites < ActiveRecord::Migration
  def up
    execute <<-EOS
      ...
      CREATE INDEX favourites_sorting_default_index ON favourites(lower(artist), lower(album), tracknr, lower(title));
      CREATE INDEX favourites_sorting_title_index ON favourites(lower(title), tracknr, lower(artist), lower(album));
      CREATE INDEX favourites_sorting_album_index ON favourites(lower(album), tracknr, lower(artist), lower(title));
    EOS
  end

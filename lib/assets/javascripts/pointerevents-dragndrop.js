...
(function($) {
  ...
  DD.prototype.bind_events = function() {
    ...
    events.push(["touchend", this.touch_end_handler]);
  ...
  DD.prototype.pointer_move_handler = function(e, via_pointer_start) {
    var eo, start_time, now_time, diff_time, diff_time_requirement,
        start_pos, now_pos, diff_pos, diff_scroll;
    ...
    if (this.state.dragging) {
      ...
      diff_time_requirement = this.settings.time_until_drag;
      ...
      if (diff_pos <= 15 && diff_scroll === 0 && this.state.amount_of_touches < 2) {
        ...
        if (via_pointer_start) {
          this.start_drag(eo);
        } else if (diff_time >= diff_time_requirement) {
          this.clear_timeouts();
          this.start_drag(eo);
        }
  ...
  DD.prototype.touch_end_handler = function(e) {
    this.stop_drag(e);
  };

...
(function($) {
  ...
  DD.prototype.pointer_down_handler = function(e) {
    ...
    // touch action map
    touch_action_map = this.collect_parent_nodes(e.currentTarget);
    touch_action_map = $(touch_action_map).filter("[touch-action]");
    touch_action_map = touch_action_map.map(function() {
      return { original: this.getAttribute("touch-action"), el: this };
    }).get();
  ...
  DD.prototype.pointer_move_handler = function(e, via_pointer_start) {
    ...
    if (this.state.dragging) {
      ...
      if (diff_pos <= 15 && this.state.amount_of_touches < 2) {
        e.preventDefault();
